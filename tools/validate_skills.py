...
def check_skill(skill_path, schemas=None, tool_registry=None, technique_skills=None):
    ...
    # --- OSMEDEUS-INSPIRED LINTER RULES ---
    if yaml_data:
        workflows = yaml_data.get("workflows", {}) or {}
        # duplicate-step-name
        wf_names = list(workflows.keys())
        if len(wf_names) != len(set(wf_names)):
            add_check("FAIL: duplicate workflow names")
            report["deductions"].append("Duplicate workflow names")
            deduct_ss("workflows", 2)
            add_issue("warning", "duplicate_step_name", "Duplicate workflow names found", str(skill_yaml))
        else:
            add_check("PASS: no duplicate workflow names")
        # empty-step
        ...
        # invalid-goto and circular-dependency via next links
        ...
        for n, w in workflows.items():
            ...
            for key in ("if_findings", "if_no_findings"):
                tgt = nxt.get(key)
                if tgt and tgt not in wf_set and tgt != "null":
                    add_check(f"FAIL: {n} next.{key}='{tgt}' not a valid workflow")
                    report["deductions"].append(f"Invalid goto: {n} -> {tgt}")
                    deduct_ss("workflows", 1)
                    add_issue("warning", "invalid_goto", f"Workflow '{n}' goto '{tgt}' invalid", str(skill_yaml))
                elif tgt and tgt in wf_set:
                    targets.append(tgt)
            graph[n] = targets
        ...
        # undefined-variable: check {{var}} in commands against known inputs + built-ins
        import re as _re
        ...
        for n, w in workflows.items():
            ...
            for var in var_re.findall(cmd):
                if var not in builtins and var not in (w.get("inputs") or []):
                    # allow extra known vars like RATE_LIMIT, etc.
                    if var not in {"RATE_LIMIT", "CONCURRENCY", "USER_AGENT", "AUTH_HEADER", "COOKIE_JAR"}:
                        add_check(f"WARN: {n} uses undefined variable {{{{{var}}}}}")
                        report["deductions"].append(f"Undefined variable in {n}: {var}")
                        deduct_ss("workflows", 1)
                        add_issue("warning", "undefined_variable", f"Workflow '{n}' uses undefined {{{{{var}}}}}", str(skill_yaml))
                        break
        # unused-variable: inputs never referenced in command
        for n, w in workflows.items():
            inputs = w.get("inputs") or []
            cmd = str(w.get("command", ""))
            for inp in inputs:
                if inp not in cmd and "{{" + inp + "}}" not in cmd and f"${inp}" not in cmd:
